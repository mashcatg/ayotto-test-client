...
const Topics = () => {
	...
	const handleTopicsSubmit = async (e) => {
		...
		const chapterId = form.chapter.value;
		const topicData = {
			name,
			chapterId,
			createdBy: user?._id,
		...
		try {
			const res = await axiosPublic.post("/topic", topicData);
			if (res.data.success) {
	...
	useEffect(() => {
		const fetchedTopics = async () => {
			try {
				const res = await axiosPublic.get("/topic");
        ...
        `}
					...
					<tbody>
						{topics?.map((topic, index) => (
							<tr key={topic._id} className="">
								...
								<td className="p-3">
									<p>{topic?.chapterId?.subjectId?.batchId?.name || "N/A"}</p>
								</td>
								<td className="p-3">
									<p>{topic?.chapterId?.subjectId?.groupId?.name || "N/A"}</p>
								</td>
								<td className="p-3">
                                    <p>{topic?.chapterId?.subjectId?.name}</p>
                                </td>
								<td className="p-3">
									<p>{topic?.chapterId?.name}</p>
								</td>
			...
			<Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
				<div className="space-y-4">
					...
					<form onSubmit={handleTopicsSubmit} className="space-y-5">
						<input
							type="text"
							name="name"
